...
class Baseline(object):
    ...
    def unwrap_batch(self, batch):
        # return list(zip(*batch['seq'])), batch['vec'], None
        return batch['vec'], None
...
class RolloutBaseline(Baseline):
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        if dataset is not None:
            if len(dataset) != self.opts.val_size:
                ...
                dataset = None
            elif (dataset[0] if self.problem.NAME == 'phylo' else dataset[0]['loc']).size(0) != self.opts.graph_size:
                print("Warning: not using saved baseline dataset since graph_size does not match")
                dataset = None

        if dataset is None:
            self.dataset = self.problem.make_dataset(phy_size=self.opts.graph_size, num_samples=self.opts.val_size)
        else:
    ...
    def unwrap_batch(self, batch):
        ...
        # bl_val: (batch_size)
        # return list(zip(*batch['seq'])), batch['vec'], batch['baseline'].view(-1)  # Flatten result to undo wrapping as 2D
        return batch['vec'], batch['baseline'].view(-1)  # Flatten result to undo wrapping as 2D
...
class BaselineDataset(Dataset):
    ...
    def __init__(self, dataset=None, bl_val=None):
        super(BaselineDataset, self).__init__()

        self.dataset = dataset
        self.bl_val = bl_val
        assert (len(self.dataset) == len(self.bl_val))

    def __getitem__(self, idx):
        return {
            'vec': self.dataset[idx],
            'baseline': self.bl_val[idx]
        }
