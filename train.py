...
from nets.models import AttentionModel, set_decode_type
...
def train_epoch(model, optimizer, baseline, lr_scheduler, epoch, val_dataset, problem, tb_logger, opts):
    ...
    # step: the number of batch have been trained.
    ...
    training_dataset = baseline.wrap_dataset(problem.make_dataset(
        phy_size=opts.graph_size, num_samples=opts.epoch_size))
    # training_dataset = problem.make_dataset(phy_size=opts.graph_size, num_samples=opts.epoch_size)
...
def train_batch(
    ...
    bl_val, bl_loss = baseline.eval(vec, cost) if bl_val is None else (bl_val, 0)
